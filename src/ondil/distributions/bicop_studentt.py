...
def _log_likelihood_t(y, rho, nu):
    ...
    u = np.array([st.t.ppf(y_clipped[i, 0], df=nu[i] if hasattr(nu, '__len__') else nu).squeeze() for i in range(M)])
    v = np.array([st.t.ppf(y_clipped[i, 1], df=nu[i] if hasattr(nu, '__len__') else nu).squeeze() for i in range(M)])
    ...
    for m in range(M):
        ...
        quad_form = (t1*t1 + t2*t2 - 2.0*rho_m*t1*t2) / (nu_m * (1-rho_m**2))
...
def _derivative_1st_nu(y, rho, nu):
    ...
    for m in range(M):
        ...
        t11 = 1.0 - rho_m * rho_m + 2.0 * x1 * out1 + 2.0 * x2 * out2 - 2.0 * rho_m * (x1 * out2 + x2 * out1)
...
def _derivative_2nd_nu(y, rho, nu):
    ...
    for m in range(M):
        ...
        M_nu_nu = (2.0 * out1 * out1 + 2.0 * x1 * out3 + 2.0 * out2 * out2 + 
                   2.0 * x2 * out4 - 4.0 * rho_m * out1 * out2 - 
                   2.0 * rho_m * (x2 * out3 + x1 * out4))
        
        deriv[m] = c * (t10 + 0.5 * (t13 + t16) + t1 * (t18 - t21 + t20 - t22) + 
                       0.5 * t13 + 0.5 * t16 - M_nu / M_val - 
                       (nu_m / 2.0 + 1.0) * (M_nu_nu / M_val - M_nu * M_nu / M_val / M_val)) + diff_nu * diff_nu / c
...
from math import log, pow, sqrt, fabs, exp
...
def incompleBeta_an1_bn1_p(x: float, p: float, q: float) -> Tuple[List[float], List[float]]:
    t2 = 1.0/(1.0 - x)
    t3 = x*t2
    t4 = q - 1.0
    t5 = p + 1.0
    ...
    bn2 = (2.0*t36 - 2.0*t40*t44 - 2.0*t40*t48
            + 2.0*t32/t42/p*t35
            + 2.0*t32*t43*t47
            + 2.0*t32*t33/t46/t34)
...
def incompleBeta_an1_bn1_q(x: float, p: float, q: float) -> Tuple[List[float], List[float]]:
    t2 = 1.0/(1.0 - x)
    t3 = x*t2
    t6 = 1.0/(p + 1.0)
    t11 = q*x*t2
...
def incompleBeta_an_bn_p(x: float, p: float, q: float, n: int) -> Tuple[List[float], List[float]]:
    t1 = x*x
    t2 = 1.0 - x
    t3 = t2*t2
    t5 = t1/t3
    ...
    t105 = (2.0*t5*t6*t29
            - 2.0*t26*t55 - 4.0*t26*t59 - 2.0*t26*t63
            - 2.0*t9*t55  - 4.0*t9*t59  - 2.0*t9*t63
    ...
    an0 = t9*t12*t19*t21
    an1 = t26*t29 + t9*t29 - t9*t12*t34*t21 - 2.0*t9*t12*t40*t21 - t9*t12*t19*t46
    an2 = t105
    bn0 = t123*t127
    bn1 = t133*t127 - t123*t135 - t123*t139
    bn2 = (2.0*t127 - 2.0*t133*t135 - 2.0*t133*t139
           + 2.0*t123*t39*t126 + 2.0*t123*t18*t138 + 2.0*t123*t124/t137/t125)
    return [an0, an1, an2], [bn0, bn1, bn2]


def incompleBeta_an_bn_q(x: float, p: float, q: float, n: int) -> Tuple[List[float], List[float]]:
    t1 = x*x
    t2 = 1.0 - x
    t3 = t2*t2
    t5 = t1/t3
    ...
    bn2 = -2.0*p*x*t39*t54*t56*t58 - 2.0*t70*t74*t59 + 2.0*t53/t73/q*t59
    return [an0, an1, an2], [bn0, bn1, bn2]


# ------------------------------ inbeder ------------------------------

def inbeder(x_in: float, p_in: float, q_in: float,
            err: float = 1e-12,
            minappx: int = 3,
            maxappx: int = 200) -> Tuple[float, float, float]:
    """
    ...
    while True:
        ...
        dr[2] = (-2*dan[1]*dbn[1] + 2*Rn*dbn[1]*dbn[1]) / dbn0_sq + (dan[2] - Rn*dbn[2]) / dbn[0]
